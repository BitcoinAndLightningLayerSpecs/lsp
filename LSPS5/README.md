...
* `too_long` (1000) - Either `app_name` or `webhook` or both are
  too long.
* `unsupported_protocol` (1001) - The `webhook` is not a protocol
...
* `too_many_webhooks` (1002) - The client already has the maximum
...
* *and* one of the following conditions is true:
  ...
  * The LSP wants to change feerates for any channels towards
    the client, or feerates for any auto-management mechanism
    the client has set up with the LSP.
...
and the webhhok notification object is the exact ASCII sequence
...
format, without a trailing `NUL` character.
...
* `lsps5.expiry_soon` - There is an HTLC or
  ...
  Parameters:
  * `timeout` - The blockheight at which the LSP would
...
* `lsps5.liquidity_management_request` - The LSP wants
  ...
  No parameters `{}`.
* `lsps5.fees_change_incoming` - The LSP wants to change
  Lightning Network feerates, either for the LSP-to-client
  channel(s), or for other auto-management services.
  Parameters:
  * `direction` - A rough estimate of the direction
    of the fees, one of the following strings:
    `"lower"`, `"mixed"`, `"higher"`.
* `lsps5.onion_message_incoming` - The client has one
  or more [BOLT Onion Message][]s pending to be received.
  No parameters `{}`.
...
delivery service needs to agree on some validation that
...
#### LSP: Rate-limit Notifications By `method`
