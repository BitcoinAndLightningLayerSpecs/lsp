...
An LSP may support extensions to the base LSPS4 protocol.

In order to signal that some extension is supported by the LSP, the
`lsps4.select_version` result will include extra keys,
corresponding to the name of the extension, with the value `true`.
[LSPS0 Method And Notification Specifications][] specifically allows
LSP call return values to include keys that are not known by the
client.

[LSPS0 Method And Notification Specifications]: ../LSPS0/README.md#lsps-method-and-notification-specifications

If a client wants to use a particular extension, the client MUST
...
The client MUST check the presence or absence of an extension
each time it negotiates the version with the LSP, which is done
at each re-connection with the LSP.

> **Rationale** The LSP may signal support for some new extension,
> but then a bug (unrelated to the extension or not) may force the
> LSP to revert to an older version that does not support this
> extension.
> Thus, a client needs to be prepared for the case where an LSP
> signals extension support, but then disconnects and then on
> reconnection no longer signals the extension as being supported.

If a client requires a specific extension, the client MAY choose
to not use an LSP that does not support that extension.

Below is a list of defined extensions:

* [`ephemeral_scids`](./ephemeral.md)

**Non-normative** For example, if an LSP supports the
`ephemeral_scids` extension, then it would give a result from
`lsps4.select_version` much like the below:
...
The client can request an SCID using the `lsps4.reserve_scid` call,
...
If this call succeeds, the LSP MUST remember the SCID as long as
ANY of the following conditions hold:

* The `expiry_time` of the SCID has not yet arrived.
* There exists at least one channel with that client, which is
  in a "normal channel operation" state:
  * After both peers have sent `channel_ready`.
  * Before any peer has sent `shutdown` or some commitment
    transaction (unilateral close) has been seen.

Once both of the above conditions are no longer true, the LSP MAY
forget the SCID.

> **Rationale** Requested SCIDs have an expiry time in order to
> time-bound any extra information the LSP has to store for
> clients that request SCIDs (which are shared among all clients
> of the same LSP) and then never actually use them.
> The expiry time gives the client an opportunity to have a
> channel opened (either by opening one itself, or otherwise
> purchasing a channel from the LSP via this LSPS or via
> [LSPS1][] or [LSPS2][]) during this expiry time period.
...
It is a positive non-zero JSON integral number.
The LSP MUST persist this together with the generated SCID if
this call succeeds.

The client may call the `lsps4.reserve_scid` any number of
times.
On the second and subsequent times, if the LSP still has
remembered a reserved SCID for this client, it MUST return the
same SCID and its stored details.
...
[<LSPS0.datetime>][].
`min_final_cltv_expiry_delta` is the final CLTV delta for the
recipient, i.e. the `c` field in BOLT11 invoices, that the LSP has
persisted together with the reserved SCID.

If the LSP has already reserved an SCID for the client, the LSP MUST
return the current SCID and its current `expiry_time` and
...
> an LSPS4 SCID, but crashes before it can persist the returned
> SCID.
...
> has also forgotten the SCID.
>
> The LSP should not change the expiry time for the LSPS4 SCID
> just because the request was repeated, and thus should return
> the original expiry time for the LSPS4 SCID, as otherwise
...
> the `expiry`/`x` field of the first BOLT11 invoice the client
> intends to issue.

Otherwise, if a fresh  SCID was created, then the `expiry_time`
and `min_final_cltv_expiry_delta` MUST be the same as the request
parameter.

`ln_base_fee_msat` is the base fee that the client MUST indicate as
the base fee for the given SCID [<LSPS0.msat>][].
...
The `ln_base_fee_msat`, `ln_prop_fee` and `cltv_expiry_delta` MAY
...
The SCID is provided to give a stable path to clients.
For example, the upcoming BOLT12 offers will need a stable SCID to
be embedded in a blinded route in order to generate a reusable
BOLT12 offer.
...
The LSPS4 SCID also allows a client to generate an invoice without
being online and connected to the LSP; it can simply cache the
LSPS4 SCID with the LSP (as it is permanent), and use it to
...
> **Rationale** The LSPS4 SCID represents all channels between
> the LSP and the client, independently of the lifetimes of
> individual channels; the LSPS4 SCID remains valid across
> closures, re-openings, and splicings of channels between the
> LSP and the client.
> If there are no channels yet, then the LSPS4 SCID represents
> the possibility of a future channel between the LSP and the
> client.
...
* The sum of `timestamp` and `x`/`expiry` fields is equal to
  or less than the `expiry_time` **IF** the client has no
  channels with the LSP.
  * If a channel exists with the LSP, then the `x` field can be
    any value selected by the client.
...
> the new current blockheight.
> The client can determine this from the notification that the
> LSP sends to the client, and this specification includes an API
> the client can use to fail that, however, there is still a race
> condition if the incoming payment is used to pay for a new channel
> open: between the time that the client checks the CLTV expiry,
> and the time that the LSP is able to open the channel and send the
> HTLC, a new block can be mined.
> This is a time-of-check vs. time-of-use race condition.
>
> A client also cannot check if it already has enough inbound
> liquidity to not buy a channel, and thus leave off the +2
> margin in that case: by the time the sender actually gets a
> payment to the client, the inbound liquidity might have changed
> (the LSP might have closed a channel to the client, for example).
> This is again a time-of-check vs. time-of-use race condition.
>
> By *always* adding a small +2 margin here, we greatly reduce the
> incidence of such an accident.
...
messages that indicate the LSPS4 SCID returned from an
`lsps4.reserve_scid` call, to the client that requested that
SCID.

Clients that want to receive onion messages SHOULD use the
LSPS4 SCID returned by `lsps4.reserve_scid`.

#### Lightning Forwarding Feerate Change

The LSP MAY change the in-Lightning forwarding fee for LSPS4
SCIDs at any time.
If so, the LSP MUST inform the client via the
`lsps4.i_changed_fee_rates` notification, whose
...
has requested a  SCID via `lsps4.reserve_scid`.
The LSP MUST NOT send this notification if the client has not
requested a LSPS4 SCID, or if the SCID has been forgotten due
to both passing the expiry time and having no more channels
with the client.

All the above fields are required.

`lsps4_scid` is the LSPS4 SCID [<LSPS0.scid>][].
...
LSPS4 SCID was first created.

`ln_base_fee_msat` is the base fee that the client MUST indicate
as the base fee for the given SCID [<LSPS0.msat>][].
`ln_prop_fee` is the proportional fee the client MUST indicate as
the proportional / parts-per-million fee for the given SCID
[<LSPS0.ppm>][].
`cltv_expiry_delta` is the CLTV difference between the LSP-inbound
...
If the LSP does not ever change fee rates, then the LSP does not
need to send this notification on reconnection.
